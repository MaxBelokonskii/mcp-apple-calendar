...
поиск свободных слотов и детектор конфликтов.
...
- `src/` — MCP-сервер: `helper.ts` (вызов бинарника), `schemas.ts` (zod),
  `format.ts` (форматирование), `availability.ts` (свободные слоты и конфликты),
  `tools/` (инструменты), `index.ts` (точка входа).
